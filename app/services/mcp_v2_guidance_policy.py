...
def apply_guidance_tool_policy(tools: list[dict]) -> None:
    ...
    for definition in (GET_SERVER_INFO_TOOL, GET_INDUCTION_TOOL):
        if definition["name"] not in by_name:
            ...
            if "securitySchemes" in read_template: tool["securitySchemes"] = read_template["securitySchemes"]
            if "_meta" in read_template: tool["_meta"] = read_template["_meta"]
            tools.insert(0, tool)
    for item in tools:
        if item.get("name") not in WRITE_TOOL_NAMES: continue
        ...
        if "version_check" not in required: required.append("version_check")
    ...
    if contribution:
        ...
        if "vote" not in enum: enum.append("vote")
    ...
    if create: create["description"] += " To propose an induction-guidance change, set context.governance_kind='induction_guidance', context.guidance_key to the stable section key, context.guidance_scope to 'global' or 'model', and context.target_model when scope is model. The proposal remains inactive until explicit user approval."
    resolution = by_name.get("record_resolution")
    if resolution: resolution["description"] += " For an induction-guidance deliberation, a successful user-approved resolution becomes active guidance returned by get_induction; AI votes alone have no activation authority."


def install_get_induction_middleware(app, mcp_module) -> None:
    @app.middleware("http")
    async def governed_induction(request, call_next):
        if request.method == "GET" and request.url.path == "/version": return JSONResponse(_server_info(mcp_module))
        if request.method != "POST" or request.url.path != "/mcp-v2": return await call_next(request)
        raw = await request.body()
        try: body = json.loads(raw or b"{}")
        except (TypeError, ValueError, json.JSONDecodeError): body = {}
        params = body.get("params") or {}; method = body.get("method"); tool_name = params.get("name")
        if method == "tools/call" and tool_name in WRITE_TOOL_NAMES:
            args = params.get("arguments") or {}; supplied = args.get("version_check"); expected = _server_info(mcp_module)["write_version_token"]
            if supplied != expected:
                result = _version_error(mcp_module, supplied=supplied)
                return JSONResponse({"jsonrpc": "2.0", "id": body.get("id"), "result": result})
            clean_body = dict(body); clean_params = dict(params); clean_args = dict(args); clean_args.pop("version_check", None)
            clean_params["arguments"] = clean_args; clean_body["params"] = clean_params; raw = json.dumps(clean_body, separators=(",", ":")).encode("utf-8")
        ...
        rpc_id = body.get("id"); args = params.get("arguments") or {}
        try: principal = mcp_module._principal(request, "reviews:read")
        except TokenError as exc: result = mcp_module._auth_error(str(exc))
        else:
            if tool_name == "get_server_info": result = mcp_module._result(_server_info(mcp_module))
            elif principal.user_id is None: result = mcp_module._error("Authenticated TestGraph user is required")
            else:
                with SessionLocal() as db: induction = get_induction(db, owner_id=principal.user_id, source_model=args.get("source_model"))

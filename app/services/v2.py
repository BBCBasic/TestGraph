...
def add_type_relationship(
    db: Session, source_type: SubjectType, relationship: str, target_type: SubjectType, *,
    source: str, commit: bool = True,
) -> TypeRelationship:
...
def ensure_subject_hierarchy(db: Session, terms: list[str], *, created_by: str) -> dict:
    """Resolve/create one broad-to-specific hierarchy without deriving meaning from insertion order.

    The AI/client is responsible for semantic judgement after inspecting vocabulary_index.
    The backend is responsible for dictionary reuse, provisional creation, relationship
    validation and atomicity. A previously unknown term may not be created as an isolated
    root because that is exactly the order-dependent failure mode this function prevents.
    """
    ...
    if len(cleaned) == 1 and resolved_before[0] is None:
        raise ValueError(
            ...
            "Inspect the vocabulary and provide a broad-to-specific hierarchy, for example food -> recipe."
        )
    ...
    try:
        ...
        for term in cleaned:
            resolved.append(ensure_subject_type(
                db, term, created_by=created_by, create_if_missing=True, commit=False,
            ))
        ...
        for parent_result, child_result in zip(resolved, resolved[1:]):
            ...
            relationships.append(add_type_relationship(
                db, child, "belongs_to", parent, source=created_by, commit=False,
            ))
        ...
        return {
            ...
            "created_terms": [subject_type.canonical_name for subject_type, created, _ in resolved if created],
            "relationships": [
                {

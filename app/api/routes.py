...
@router.post("/api/v1/subjects", response_model=SubjectRead)
...
def list_subjects(subject_type:str|None=None, limit:int=50, offset:int=0, db:Session=Depends(get_db)):
    q=select(Subject).where(Subject.deleted_at.is_(None))
    if subject_type:q=q.where(Subject.subject_type==subject_type)
    return list(db.scalars(q.offset(offset).limit(min(limit,100))).all())

@router.post("/api/v1/experiences/drafts", response_model=ExperienceRead, status_code=201)
...
def list_experiences(subject_id:uuid.UUID|None=None, owner_id:uuid.UUID|None=None, subject_type:str|None=None, publication_status:str|None=None, limit:int=50, offset:int=0, db:Session=Depends(get_db)):
...
@router.post("/api/v1/experiences/{experience_id}/publish", response_model=ExperienceRead)
...
@router.delete("/api/v1/experiences/{experience_id}", status_code=204)
...
@router.get("/api/v1/experiences/{experience_id}/for/{reader_id}", response_model=PersonalisedReview)
...
@router.get("/schemas/{subject_type}/{version}")

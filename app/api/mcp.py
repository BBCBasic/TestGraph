...
def _tool_security(schemes: list[dict]) -> dict:
    # securitySchemes is current MCP Apps metadata; _meta alias keeps compatibility
    # with clients that implemented the earlier OpenAI Apps SDK shape.
...
TOOLS = [
    {
        ...
        "description": "Use this when the user asks what they have reviewed, wants recent reviews, wants a prior experience, or needs taste evidence for a recommendation. Query may be omitted to return recent reviews.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "default": "", "description": "Optional words from the place, dish, headline or review. Leave blank for recent reviews."},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"id": {"type": "string", "description": "TasteGraph experience ID returned by search."}},
            "required": ["id"],
            "additionalProperties": False,
        },
        **_tool_security(READ_SECURITY),
    ...
    },
    {
        ...
        "description": "Use this before saving when you need the exact structured fields supported for a recipe or restaurant review.",
        "inputSchema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Use this only after the user has explicitly approved the completed review. It resolves or creates the subject, saves and publishes the review, and safely handles retries with the idempotency key.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject_type": {"type": "string", "enum": ["recipe", "restaurant"]},
                ...
                "common_data": {"type": "object"},
                "domain_data": {"type": "object"},
                "visibility": {"type": "string", "enum": ["private", "unlisted", "public", "aggregate_only"], "default": "private"},
                "user_approved": {"type": "boolean", "description": "Must be true only after explicit user approval in the conversation."},
                ...
                "source_client": {"type": "string", "maxLength": 120, "default": "mcp-client", "description": "Client name when known, for provenance only (for example chatgpt or claude)."},
            },
...
def _search(db: Session, principal: Principal, args: dict):
    ...
    if args.get("subject_type"):
        q = q.where(Experience.subject_type == args["subject_type"])
    if query:
        pattern = f"%{query}%"
        q = q.where(or_(
...
def _fetch(db: Session, principal: Principal, args: dict):
    try:
        ...
        return {"content": _text({"error": "Invalid review ID"}), "isError": True}
    ...
    if not row:
        return {"content": _text({"error": "Review not found"}), "isError": True}
    exp, subject = row
    payload = {
        "id": str(exp.id),
        "title": f"{subject.name} — {exp.headline}",
        ...
        "metadata": {
            "subject": {
                ...
                "identifiers": subject.canonical_identifiers,
...
def _get_schema(db: Session, args: dict):
    ...
    if not row:
        return {"content": _text({"error": "Schema not found"}), "isError": True}
    return _result({
...
def _save(db: Session, principal: Principal, args: dict, request_id: str):
    ...
    subject = db.scalar(select(Subject).where(
        Subject.subject_type == args["subject_type"],
    ...
    exp = create_experience(
        db, payload, client_id=principal.client_id, auth_subject=principal.subject, request_id=request_id
    )
    exp = publish_experience(
        db, exp, 1, actor_id=principal.subject, client_id=principal.client_id, request_id=request_id
    )
    body = {
        "saved": True,
...
async def mcp(request: Request, db: Session = Depends(get_db)):
    ...
    if method == "initialize":
        result = {
            ...
            "instructions": "TasteGraph is the connected user's structured review memory. Search and fetch prior reviews when relevant. Use get_schema when structured fields are needed. Save only a completed review that the user has explicitly approved.",
        }
    elif method == "ping":
        result = {}
        ...
        try:
            ...
            result = _auth_result(str(exc), required)
        else:
            if name == "search":

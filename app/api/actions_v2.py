...
from app.services.v2 import create_assessment, create_experience, ensure_subject, normalise_path, propose_concept_fields, vocabulary
...
def concept(path: str | None = None, authorization: str | None = Header(None, alias="Authorization"), db: Session = Depends(get_db)):
    ...
    if not obj:
        pending=db.scalar(select(Concept).where(Concept.path==canonical,Concept.status=="pending"))
        return JSONResponse({"found":False,"path":canonical,"pending":bool(pending),"canonical_route":_concept_route(canonical),"root_route":_concept_route(),"instruction":"This path is pending independent verification." if pending else "Concept not found. Search the global vocabulary index and browse from root before proposing a genuinely new path."},headers=_headers())
...
def concept_field_proposal(payload:dict, authorization:str|None=Header(None,alias="Authorization"), db:Session=Depends(get_db)):
    ...
    try:
        ...
        concept=ensure_proposed_concept(db,ConceptEnsure(path=payload["concept_path"],description=payload.get("concept_description"),created_by=client_id))
        rows=propose_concept_fields(db,concept=concept,proposals=proposals,proposer_client_id=client_id)
        body={"concept_path":concept.path,"concept_status":concept.status,"concept_version":concept.version,"proposals":[{"id":str(row.id),"canonical_name":row.canonical_name,"json_schema":row.json_schema,"status":row.status} for row in rows],"verification_required":True,"instruction":"A different authenticated AI should find the pending path/words through the global vocabulary index, inspect the proposal, and call verifyTasteGraphV2ConceptFieldProposal if it agrees. The proposing client cannot self-verify.","manual_approval_url":f"{_base()}/development/concept-fields","experience_created":False}
        db.commit()
...
def verify_concept_field(proposal_id: uuid.UUID, payload:dict | None = None, authorization:str|None=Header(None,alias="Authorization"), db:Session=Depends(get_db)):
    cred=_auth(db,authorization); client_id=f"capability:{cred.id}:v2"
    try:
        proposal, field = verify_field_proposal(db, proposal_id, verifier_client_id=client_id, reason=(payload or {}).get("rationale")); concept = db.get(Concept, proposal.concept_id)
    except ValueError as exc:
        db.rollback(); raise HTTPException(422,str(exc))
...
def alias_proposal(payload:dict, authorization:str|None=Header(None,alias="Authorization"), db:Session=Depends(get_db)):
    cred=_auth(db,authorization)
    try:
        path=normalise_path(payload["concept_path"]); obj=db.scalar(select(Concept).where(Concept.path==path,Concept.status=="active"))
        if not obj: raise HTTPException(404,"Concept not found")
        status=propose_alias(db,concept=obj,alias=payload["alias"],canonical_name=payload["canonical_name"],proposer_client_id=f"capability:{cred.id}:v2",confidence=payload.get("confidence"),rationale=payload.get("rationale")); db.commit()
    except KeyError as exc:
        db.rollback(); raise HTTPException(422,f"Missing field: {exc.args[0]}")
    except ValueError as exc:
        db.rollback(); raise HTTPException(422,str(exc))
...
def save(payload:dict, authorization:str|None=Header(None,alias="Authorization"), db:Session=Depends(get_db)):
    ...
    try:
        ...
        if not concept: raise ValueError("Concept is not canonical yet; search the vocabulary index, then have another AI verify the pending proposal before saving")
...
def openapi():
    ...
    concept_field_proposal={"type":"object","additionalProperties":False,"required":["concept_path","fields"],"properties":{"concept_path":{"type":"string","description":"Proposed canonical dotted path after searching the index and browsing the existing tree."},"concept_description":{"type":"string"},"fields":{"type":"array","minItems":1,"items":field}}}
    verification={"type":"object","additionalProperties":False,"properties":{"rationale":{"type":"string"}}}
    ...
    assessment={"type":"object","additionalProperties":False,"required":["experience_id","assessment_type","idempotency_key"],"properties":{"experience_id":{"type":"string","format":"uuid"},"assessment_type":{"type":"string"},"evidence":{"type":"object","additionalProperties":True},"analysis":{"type":"object","additionalProperties":True},"conclusion":{"type":"string"},"confidence":{"type":"number","minimum":0,"maximum":1},"source_model":{"type":"string"},"provenance":{"type":"object","additionalProperties":True},"idempotency_key":idem}}
    return {"openapi":"3.1.0","info":{"title":"TasteGraph v2 ChatGPT Action","version":"2.4.1-alpha","description":"Cross-AI experience memory with a DNS-style canonical concept tree and a global word-to-position vocabulary index. New path and field proposals stay pending and require independent verification by a different authenticated AI before promotion."},"servers":[{"url":_base()}],"components":{"securitySchemes":{"bearerAuth":{"type":"http","scheme":"bearer"}},"schemas":{"ExperienceCreate":experience,"AssessmentCreate":assessment,"AliasProposal":alias_proposal,"ConceptFieldProposal":concept_field_proposal,"ConceptFieldVerification":verification}},"security":[{"bearerAuth":[]}],"paths":{"/actions-v2/experiences":{"get":{"operationId":"searchTasteGraphV2Experiences","summary":"Search experiences","responses":{"200":{"description":"Results"}}},"post":{"operationId":"saveTasteGraphV2Experience","summary":"Save approved direct experience","x-openai-isConsequential":True,"requestBody":{"required":True,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/ExperienceCreate"}}}},"responses":{"201":{"description":"Saved"}}}},"/actions-v2/experiences/{experience_id}":{"get":{"operationId":"fetchTasteGraphV2Experience","summary":"Fetch experience with its AI-derived assessments","parameters":[{"name":"experience_id","in":"path","required":True,"schema":{"type":"string","format":"uuid"}}],"responses":{"200":{"description":"Experience and linked assessments"}}}},"/actions-v2/concepts":{"get":{"operationId":"getTasteGraphV2Concept","summary":"Browse canonical concept nodes; pending paths are not canonical","parameters":[{"name":"path","in":"query","required":False,"schema":{"type":"string"}}],"responses":{"200":{"description":"Concept root or concept node"}}}},"/actions-v2/vocabulary-index":{"get":{"operationId":"searchTasteGraphV2VocabularyIndex","summary":"Find every canonical or pending DNS-tree position containing a word; omit word for the full index","parameters":[{"name":"word","in":"query","required":False,"schema":{"type":"string"}}],"responses":{"200":{"description":"Word-to-position index results"}}}},"/actions-v2/concept-field-proposals":{"post":{"operationId":"proposeTasteGraphV2ConceptFields","summary":"Propose a new path and fields; all new vocabulary stays pending for independent AI verification","x-openai-isConsequential":False,"requestBody":{"required":True,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/ConceptFieldProposal"}}}},"responses":{"201":{"description":"Pending proposal recorded"}}}},"/actions-v2/concept-field-proposals/{proposal_id}/verify":{"post":{"operationId":"verifyTasteGraphV2ConceptFieldProposal","summary":"Independently verify and commit another AI's pending concept/field proposal","x-openai-isConsequential":False,"parameters":[{"name":"proposal_id","in":"path","required":True,"schema":{"type":"string","format":"uuid"}}],"requestBody":{"required":False,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/ConceptFieldVerification"}}}},"responses":{"200":{"description":"Pending path and field promoted to canonical vocabulary"},"422":{"description":"Invalid proposal or self-verification attempt"}}}},"/actions-v2/alias-proposals":{"post":{"operationId":"proposeTasteGraphV2Alias","summary":"Propose that a term means an existing canonical field","x-openai-isConsequential":False,"requestBody":{"required":True,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/AliasProposal"}}}},"responses":{"201":{"description":"Proposal recorded or consensus alias accepted"}}}},"/actions-v2/assessments":{"post":{"operationId":"saveTasteGraphV2Assessment","summary":"Save routine AI-derived analysis against an exact experience; linkage and provenance are automatic","x-openai-isConsequential":False,"requestBody":{"required":True,"content":{"application/json":{"schema":{"$ref":"#/components/schemas/AssessmentCreate"}}}},"responses":{"201":{"description":"Saved and automatically linked"}}}}}}

...
from app.services.v2 import create_assessment, create_experience, ensure_subject, list_field_proposals, normalise_path, propose_concept_fields, vocabulary
...
def _security(schemes): return {"securitySchemes": schemes, "_meta": {"securitySchemes": schemes}}
...
def _proposal_schema():
    return {"type":"object","properties":{"submitted_name":{"type":"string"},"canonical_name":{"type":"string"},"json_schema":{"type":"object","description":"Complete JSON Schema for this field, including object properties and array items.","additionalProperties":True},"description":{"type":"string"},"aliases":{"type":"array","items":{"type":"string"}}},"required":["submitted_name","canonical_name","json_schema"],"additionalProperties":False}
...
def _fetch(db, principal, args):
    ...
    e,s,c=row; assessments=list(db.scalars(select(Assessment).where(Assessment.experience_id==e.id).order_by(Assessment.created_at)).all())
...
def _get_concept(db, args):
    ...
    if not concept:
        ...
        return _error("Concept not canonical",{"path":path,"pending":bool(pending),"instruction":"Search vocabulary_index; if pending, a different AI should verify the proposal."})
...
def _verify_concept_field_proposal(db, principal, args):
    ...
    proposal,field=verify_field_proposal(db,proposal_id,verifier_client_id=f"{principal.client_id}:v2",reason=args.get("rationale")); concept=db.get(Concept,proposal.concept_id)
...
def _propose_alias(db, principal, args):
    path=normalise_path(str(args.get("concept_path",""))); concept=db.scalar(select(Concept).where(Concept.path==path,Concept.status=="active"))
    if not concept: return _error("Concept not found",{"path":path})
    status=propose_alias(db,concept=concept,alias=str(args.get("alias","")),canonical_name=str(args.get("canonical_name","")),proposer_client_id=f"{principal.client_id}:v2",confidence=args.get("confidence"),rationale=args.get("rationale")); db.commit(); return _result(status)
...
def _save_experience(db, principal, args):
    ...
    relevant={k:v for k,v in args.items() if k!="idempotency_key"}; client_id=f"{principal.client_id}:v2"
    ...
    finish_idempotent_write(db,client_id=client_id,key=f"experience:{args['idempotency_key']}",payload_hash=payload_hash,response_body=body); return _result(body)
...
def _save_assessment(db, principal, args):
    ...
    finish_idempotent_write(db,client_id=client_id,key=f"assessment:{args['idempotency_key']}",payload_hash=payload_hash,response_body=body); return _result(body)

...
from app.services.workflows import preflight_existing_subject_mutation

router = APIRouter()
PROTOCOL_VERSION = "2025-06-18"
SERVER_VERSION = "3.24.0-alpha"
...
def _classification_prerequisite_error(subject: V2Subject, workflow: dict):
    disputed = workflow.get("state") == "disputed"
    code = (
    ...
    payload = {
        "error": (
            "Classification resolution is required before this subject can be updated"
            if disputed
            else "Classification review is required before this subject can be updated"
        ),
        "error_code": code,
        "workflow": workflow,
        "details": {
            ...
            "classification_status": subject.classification_status,
            "instruction": (
                "Complete the returned classification workflow, then retry the unchanged "
                "mutation with the same deterministic idempotency key. The requested mutation "
...
def _preflight_existing_subject_mutation(db, principal, subject):
    workflow = preflight_existing_subject_mutation(
        db,
        subject,
        owner_id=principal.user_id,
        actor_client=principal.client_id,
    )
    if workflow is None:
        return None
    return _classification_prerequisite_error(subject, workflow)
...
TOOLS = [
    ...
    {"name": "vocabulary_index", "title": "Export complete vocabulary", "description": "Administrative and debugging export of every canonical subject type, alias, relationship and reusable field. Normal AI classification and retrieval must use the bounded root, child and path navigation tools instead.", "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False}, **_security(READ_SECURITY), "annotations": {"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False}},
    {
        "name": "list_root_subject_types",
        "title": "List root subject types",
        "description": "Start bounded vocabulary traversal here when a direct type lookup is insufficient. Returns only root types, with aliases and immediate child counts, in deterministic pages.",
        "inputSchema": {"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 100, "default": 50}, "cursor": {"type": "string"}}, "additionalProperties": False},
        **_security(READ_SECURITY),
        "annotations": {"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False},
    },
    {
        "name": "list_child_subject_types",
        "title": "List immediate child subject types",
        "description": "Continue bounded vocabulary traversal through one candidate branch. Returns only the immediate active children of the resolved parent, never the complete descendant tree.",
        "inputSchema": {"type": "object", "properties": {"parent": {"type": "string", "minLength": 1}, "limit": {"type": "integer", "minimum": 1, "maximum": 100, "default": 50}, "cursor": {"type": "string"}}, "required": ["parent"], "additionalProperties": False},
        **_security(READ_SECURITY),
        "annotations": {"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False},
    },
    {
        "name": "get_subject_type_path",
        "title": "Get subject type parent paths",
        "description": "Return the active root-to-type path, immediate parents and compact local type details for one canonical name or alias. Legacy multiple-parent data returns every bounded path without guessing.",
        "inputSchema": {"type": "object", "properties": {"subject_type": {"type": "string", "minLength": 1}}, "required": ["subject_type"], "additionalProperties": False},
        **_security(READ_SECURITY),
        "annotations": {"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False},
    },
...
def _list_child_subject_types(db, args):
    try:
        ...
        return _error(str(exc), {"code": "VOCABULARY_BRANCH_INVALID"})
    ...
    if unresolved_types:
        return None, _error(
            "Subject context contains unresolved subject types",
            {
                ...
                "instruction": (
                    "Use progressive root/child navigation for every unknown subject type. Reuse an adequate "
                    "existing type or call resolve_subject_hierarchy before retrying the unchanged request."
                ),
            },
        )
...
def _enrich_subject(db, principal, args):
    ...
    prerequisite = _preflight_existing_subject_mutation(db, principal, subject)
...
def _save_experience(db, principal, args):
    ...
    if not subject_type:
        return _error(
            ...
            {"instruction": "Use list_root_subject_types and list_child_subject_types with ranked fallbacks. Reuse an adequate existing type or call resolve_subject_hierarchy with the verified broad-to-specific path before resubmitting this review."},
        )
    existing_subject = db.scalar(select(V2Subject).where(
        V2Subject.subject_type_id == subject_type.id,
        V2Subject.canonical_key == args["canonical_key"],
        V2Subject.deleted_at.is_(None),
    ))
    if existing_subject is not None:
        prerequisite = _preflight_existing_subject_mutation(
            db, principal, existing_subject,
        )
        if prerequisite is not None:
            return prerequisite
...
def _assert_location(db, principal, args):
    ...
    if prior is not None:
        return _result(prior)
    subject, _, locator_error = _locate_subject_for_write(
        db, {"subject_id": args.get("subject_id")}
    )
    if locator_error is not None:
        return locator_error
    ...
    body = {
        "saved": True,
        "subject_id": str(subject.id),
        "assertion": assertion_body(db, item),
    }
    finish_idempotent_write(
    ...
    if method == "initialize":
        ...
        try:
            ...
            try:
                ...
                elif name == "list_root_subject_types": result = _list_root_subject_types(db, args)
                elif name == "list_child_subject_types": result = _list_child_subject_types(db, args)
                elif name == "get_subject_type_path": result = _get_subject_type_path(db, args)

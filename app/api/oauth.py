...
def _owner(db: Session) -> User:
    ...
    try:
        ...
        raise HTTPException(503, "OAUTH_OWNER_USER_ID is invalid") from exc
    user = db.get(User, user_id)
    if not user or user.deleted_at:
        raise HTTPException(503, "Configured TasteGraph owner does not exist")
    return user
...
def _user_from_capability(db: Session, supplied: str) -> User | None:
    ...
    if not key.startswith("tg_"):
        return None
    cred = db.scalar(select(CapabilityCredential).where(
        CapabilityCredential.key_hash == _hash(key),
...
async def authorize_submit(request: Request, db: Session = Depends(get_db)):
    ...
    values = {
        k: str(form.get(k, ""))
        for k in ("response_type", "client_id", "redirect_uri", "state", "code_challenge", "code_challenge_method", "resource", "scope")
    }
    scope = _validate_authorize(
        db,
    ...
    supplied = str(form.get("connection_secret", ""))
    user = _connecting_user(db, supplied)
    if not user:
        return HTMLResponse(
            "<h1>Connection refused</h1><p>The TasteGraph capability URL/key was not recognised.</p>",
            status_code=401,
        )
    ...
    return RedirectResponse(
        f'{values["redirect_uri"]}?{urlencode({"code": raw_code, "state": values["state"]})}',
        status_code=303,
    )
...
def _new_refresh(db: Session, *, client_id: str, user_id: uuid.UUID, scope: str, resource: str) -> str:
    ...
    db.add(OAuthRefreshToken(
        ...
        scope=scope,
        resource=resource,
        expires_at=_now() + timedelta(days=get_settings().oauth_refresh_token_days),
    ))
    return raw
...
async def token(request: Request, db: Session = Depends(get_db)):
    ...
    if grant_type == "authorization_code":
        code = db.scalar(select(OAuthAuthorizationCode).where(
            OAuthAuthorizationCode.code_hash == _hash(str(form.get("code", "")))
        ))
        ...
        old = db.scalar(select(OAuthRefreshToken).where(
            OAuthRefreshToken.token_hash == _hash(str(form.get("refresh_token", "")))
        ))
